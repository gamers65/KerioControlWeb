...
Позволяет удобно загружать IP, домены, URL, формировать группы и одновременно обновлять Address Groups и URL Groups. Приложение для пользователей, которым приходится постоянно обрабатывать в частности письма от ФСТЭК и блокировать большое количество адресов и URL. Но и подойдет всем  желающим. Функция распознавания адресов из PDF реализована именно для писем этих регуляторов. Для работы распознавания нужно развернуть API (пункт 2 и 3).
